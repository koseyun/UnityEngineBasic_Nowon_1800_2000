...
// 멤버 변수, 멤버 함수를 가짐
// 생성자가 있음
// (단, 생성자는 매개변수로 모든 멤버 변수에 대응되는 파라미터를 가지고 있어야 하며, 생성자 호출시 모든 멤버변수는 초기화 되어야 한다)
// 왜 모든 멤버변수가 꼭 초기화 되어야 하나 ?
// 구조체는 값 형식이고 스택영역에 할당되기 때문

// 구조체 vs 클래스
// 모든 멤버의 크기 합이 16byte 를 초과할 경우 클래스를 사용한다. (최우선 조건)
// 값의 전달이 빈번하게 일어나는 경우 구조체를 쓴다. (예. 함수에 인자로 전달해야하는 경우)
